import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import Header from '../../components/ui/Header';
import Sidebar from '../../components/ui/Sidebar';
import MetricCard from './components/MetricCard';
import ActivityFeed from './components/ActivityFeed';
import QuickActions from './components/QuickActions';
import RealtimeIndicator from './components/RealtimeIndicator';

const Dashboard = () => {
  const navigate = useNavigate();
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false);
  const [metrics, setMetrics] = useState({
    monthlySales: {
      value: '€12,450',
      change: '+15.3%',
      changeType: 'positive'
    },
    orderCount: {
      value: '156',
      change: '+8.2%',
      changeType: 'positive'
    },
    newCustomers: {
      value: '23',
      change: '+12.1%',
      changeType: 'positive'
    },
    whatsappMessages: {
      value: '1,247',
      change: '-2.4%',
      changeType: 'negative'
    }
  });

  const handleSidebarToggle = () => {
    setIsSidebarCollapsed(!isSidebarCollapsed);
  };

  // Simulate real-time metric updates
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...prev,
        whatsappMessages: {
          ...prev?.whatsappMessages,
          value: String(parseInt(prev?.whatsappMessages?.value?.replace(',', '')) + Math.floor(Math.random() * 5))
        }
      }));
    }, 45000);

    return () => clearInterval(interval);
  }, []);

  const metricCards = [
    {
      title: 'Ventas del Mes',
      value: metrics?.monthlySales?.value,
      change: metrics?.monthlySales?.change,
      changeType: metrics?.monthlySales?.changeType,
      icon: 'TrendingUp',
      iconColor: 'text-success',
      actionLabel: 'Ver detalles',
      onActionClick: () => navigate('/orders-management')
    },
    {
      title: 'Pedidos Totales',
      value: metrics?.orderCount?.value,
      change: metrics?.orderCount?.change,
      changeType: metrics?.orderCount?.changeType,
      icon: 'ShoppingCart',
      iconColor: 'text-primary',
      actionLabel: 'Gestionar',
      onActionClick: () => navigate('/orders-management')
    },
    {
      title: 'Nuevos Clientes',
      value: metrics?.newCustomers?.value,
      change: metrics?.newCustomers?.change,
      changeType: metrics?.newCustomers?.changeType,
      icon: 'Users',
      iconColor: 'text-accent',
      actionLabel: 'Ver clientes',
      onActionClick: () => navigate('/customers-management')
    },
    {
      title: 'Mensajes WhatsApp',
      value: metrics?.whatsappMessages?.value,
      change: metrics?.whatsappMessages?.change,
      changeType: metrics?.whatsappMessages?.changeType,
      icon: 'MessageSquare',
      iconColor: 'text-secondary',
      actionLabel: 'Configurar',
      onActionClick: () => navigate('/whats-app-integration-settings')
    }
  ];

  return (
    <div className="min-h-screen bg-background">
      <Header onMenuToggle={handleSidebarToggle} />
      <Sidebar 
        isCollapsed={isSidebarCollapsed} 
        onToggle={handleSidebarToggle}
      />
      <main className={`pt-16 transition-all duration-300 ${
        isSidebarCollapsed ? 'lg:pl-16' : 'lg:pl-64'
      }`}>
        <div className="p-4 lg:p-6 space-y-6">
          {/* Page Header */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-foreground">Panel de Control</h1>
              <p className="text-muted-foreground">
                Bienvenido de vuelta, aquí tienes un resumen de tu tienda
              </p>
            </div>
            <div className="text-sm text-muted-foreground">
              {new Date()?.toLocaleDateString('es-ES', { 
                weekday: 'long', 
                year: 'numeric', 
                month: 'long', 
                day: 'numeric' 
              })}
            </div>
          </div>

          {/* Real-time Status Indicator */}
          <RealtimeIndicator />

          {/* Metrics Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {metricCards?.map((metric, index) => (
              <MetricCard
                key={index}
                title={metric?.title}
                value={metric?.value}
                change={metric?.change}
                changeType={metric?.changeType}
                icon={metric?.icon}
                iconColor={metric?.iconColor}
                actionLabel={metric?.actionLabel}
                onActionClick={metric?.onActionClick}
              />
            ))}
          </div>

          {/* Main Content Grid */}
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            {/* Activity Feed - Takes 2 columns on xl screens */}
            <div className="xl:col-span-2">
              <ActivityFeed />
            </div>
            
            {/* Quick Actions - Takes 1 column on xl screens */}
            <div className="xl:col-span-1">
              <QuickActions />
            </div>
          </div>

          {/* Additional Stats Row */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-card border border-border rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-medium text-muted-foreground">Productos Activos</h3>
                <div className="p-2 rounded-lg bg-muted text-primary">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z" />
                  </svg>
                </div>
              </div>
              <div className="text-2xl font-bold text-foreground">89</div>
              <p className="text-xs text-muted-foreground mt-1">5 con stock bajo</p>
            </div>

            <div className="bg-card border border-border rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-medium text-muted-foreground">Tasa de Conversión</h3>
                <div className="p-2 rounded-lg bg-muted text-success">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M3.293 9.707a1 1 0 010-1.414l6-6a1 1 0 011.414 0l6 6a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L4.707 9.707a1 1 0 01-1.414 0z" clipRule="evenodd" />
                  </svg>
                </div>
              </div>
              <div className="text-2xl font-bold text-foreground">24.8%</div>
              <p className="text-xs text-success mt-1">+3.2% vs mes anterior</p>
            </div>

            <div className="bg-card border border-border rounded-lg p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-medium text-muted-foreground">Ticket Promedio</h3>
                <div className="p-2 rounded-lg bg-muted text-accent">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z" />
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clipRule="evenodd" />
                  </svg>
                </div>
              </div>
              <div className="text-2xl font-bold text-foreground">€79.80</div>
              <p className="text-xs text-muted-foreground mt-1">Basado en últimos 30 días</p>
            </div>
          </div>
        </div>
      </main>
    </div>
  );
};

export default Dashboard;